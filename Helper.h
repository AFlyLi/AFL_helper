...
// 屏幕相关
#define AFL_SCREEN_WIDTH                 [UIScreen mainScreen].bounds.size.width
#define AFL_SCREEN_HEIGHT                [UIScreen mainScreen].bounds.size.height
#define AFL_SAVEAREA_TOP                 (AFL_IS_iPhoneX ? 24.0f : 0.0f)   // 顶部安全区域
#define AFL_SAVEAREA_BTM                 (AFL_IS_iPhoneX ? 34.0f : 0.0f)   // 底部安全区域
#define AFL_STATUSBAR_HEIGHT             (AFL_IS_iPhoneX ? 44.0f : 20.0f)  // 状态栏高度
#define AFL_NAVBAR_HEIGHT                44.0f   // 导航栏高度
#define AFL_STATUSBAR_NAVBAR_HEIGHT      (AFL_STATUSBAR_HEIGHT + AFL_NAVBAR_HEIGHT) // 状态栏+导航栏高度
#define AFL_TABBAR_HEIGHT                (AFL_IS_iPhoneX ? 83.0f : 49.0f)  //tabbar高度

// 判断是否是iPhone X系列
#define AFL_IS_iPhoneX      ([UIScreen instancesRespondToSelector:@selector(currentMode)] ?\
(\
